...
{
    [Route("api/detailedsalesreportrx")]
    [TypeFilter(typeof(CheckPermissionAttribute), Arguments = new object[] { Permission.Report })]
    public class DetailedSalesReportRxController : BaseController<DetailedSalesReportRxController>
    {
        private readonly PrintingInfo _printingInfo;

        public DetailedSalesReportRxController(IUnitOfWork unitOfWork,
             ILogger<DetailedSalesReportRxController> logger,
        ...
        [HttpGet("file", Name = "GetDetailedSalesReportFileRx")]
        public IActionResult GetDetailedSales(bool isPdf = false, DateTime? startDate = null, DateTime? endDate = null, int? terminalId = null, int? userId = null)
        {
            ...
            var detailedSalesDtos = _unitOfWork.ReportRepository.GetDetailedSalesForRx(startDate.Value, endDate.Value, terminalId, userName).ToList();
            ...
            if (isPdf)
            {
                ...
                var filecontent = ExportPDF(detailedSalesDtos.ToList(), columnsHeader, "Detailed Sales Report - Rx", startDate.Value, endDate.Value, terminalName, userName);
                return File(filecontent, "application/pdf", "DetailedSalesReport - Rx" + startDate.Value + " " + endDate.Value + ".pdf"); ;
            }
            else
        ...
        private byte[] ExportPDF(List<DetailedSalesDto> dataList, List<string> columnsHeader, string heading, DateTime startDate, DateTime endDate, string terminalName, string userName)
        {
            ...
            float[] widths = new float[] { 12f, 4f, 3f, 3f };
            ...
            {
                pdfcell = new PdfPCell(new Phrase(item.ProductName, font5))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
                table.AddCell(pdfcell);

                pdfcell = new PdfPCell(new Phrase(item.SalesDate.Value.ToString("MM/dd/yyyy"), font5))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
                table.AddCell(pdfcell);

                pdfcell = new PdfPCell(new Phrase(item.Quantity?.ToString("F2"), font5))
                {
                    HorizontalAlignment = Element.ALIGN_RIGHT
                };
                table.AddCell(pdfcell);

                pdfcell = new PdfPCell(new Phrase(item.SalesPrice?.ToString("F2"), font5))
                {
                    HorizontalAlignment = Element.ALIGN_RIGHT
                };
                table.AddCell(pdfcell);
            ...
            pdfcell = new PdfPCell(new Phrase(grandDiscount?.ToString("F2"), font5))
            {
                HorizontalAlignment = Element.ALIGN_RIGHT
            };
            table.AddCell(pdfcell);

            pdfcell = new PdfPCell(new Phrase(grandTotalSold?.ToString("F2"), font5))
            {
                HorizontalAlignment = Element.ALIGN_RIGHT
            };
            table.AddCell(pdfcell);
        ...
        private FileStreamResult ExportSalesReport(List<DetailedSalesDto> _smapleData, DateTime startDate, DateTime endDate, string terminalName, string userName)
        {
            ...
            using (var document = SpreadsheetDocument.Create(memoryStream, SpreadsheetDocumentType.Workbook))
            {
                ...
                header2row.Append(
                     new DocumentFormat.OpenXml.Spreadsheet.Cell()
                     {
                         CellValue = new CellValue("Detailed Sales Report - Rx (Terminal: " + terminalName + ",User: " + userName + ")"),
                         DataType = CellValues.String
                     }
                );
                header3row.Append(

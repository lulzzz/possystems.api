...
{
    public class TranCloudManualTransaction : TranCloudTransaction
    {
        public TStream TStream;

        public TranCloudManualTransaction(string invoice, ILogger<SalesController> logger, TranCloudConfig tranCloudConfig) : base(invoice, logger, tranCloudConfig)
        {
        }

        public override string GetTranCloudTransaction(double amount)
        {
            var tranCloudTransaction = new TranCloudManualTransaction(_invoiceNo, _logger, _tranCloudConfig)
            {
                TStream = new TStream
                {
